...
def initialize_db(mode: str = "persistent", synchronous: Optional[str] = None):
    ...
    if mode == "in-memory":
        DB_PATH = ":memory:"
        logging.debug(f"DB_PATH set to: {DB_PATH} (in-memory mode).")
    else: # persistent
        os.makedirs(DB_DIR, exist_ok=True)
        DB_PATH = os.path.join(DB_DIR, "entities.db")
        logging.debug(f"DB_PATH set to: {DB_PATH} (persistent mode).")
    ...
    if not DB_WRITER_THREAD or not DB_WRITER_THREAD.is_alive():
        logging.info("DB writer thread not running, creating a new one.")
        DB_WRITER_THREAD = threading.Thread(target=_db_writer, daemon=True)
        DB_WRITER_THREAD.start()
        logging.info("DB writer thread started.")
    else:
        logging.debug("DB writer thread is already running.")
...
def shutdown_db_writer():
    ...
    logging.info("Signaling DB writer thread to shut down.")
    ...
    if DB_WRITER_THREAD:
        ...
        logging.debug(f"DB writer thread status after join: {'alive' if DB_WRITER_THREAD and DB_WRITER_THREAD.is_alive() else 'terminated'}.")
    ...
    if not DB_WRITE_QUEUE.empty():
        ...
        logging.info("DB write queue is empty.")
