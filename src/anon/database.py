...
class DatabaseContext:
    ...
    def shutdown(self):
        ...
        if not self.is_initialized or self._shutdown_flag.is_set():
            logging.info("DatabaseContext not initialized or already shutting down, skipping shutdown.")
            return

        logging.info("Shutting down DatabaseContext. Signaling writer thread to stop...")
        self._shutdown_flag.set()  # 1. Prevent new items from being added

        # 2. Send sentinel to the queue to signal the writer thread to stop processing new items.
        self.write_queue.put(None)

        # 3. Wait for the writer thread to finish its current work.
        ...
        # 4. After the thread has terminated, handle any items that might be left in the queue.
        ...
        # 5. Close the repository connection.
        if self.repository:
    ...
    def save_entities(self, entity_list: List[Tuple]):
        ...
        if self._shutdown_flag.is_set():
            logging.warning("Database context is shutting down. Skipping save for %d entities and logging to dead-letter.", len(entity_list))
        ...
        if not self.is_initialized or (self.writer_thread and not self.writer_thread.is_alive()):
            logging.warning("Database context is not ready or writer thread is dead. Skipping save for %d entities.", len(entity_list))
            self._log_to_dead_letter(entity_list)
            return
