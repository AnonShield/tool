...
def compile_csvs(base_dir: Path, output_file: Path):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compila todos os CSVs em subpastas de um diretório base"
    )
    parser.add_argument(
        "input_dir",
        nargs="?",
        default="/home/kapelinski/Documents/tool/vulnnet_scans_openvas",
        help="Diretório base com os scans CSV em subpastas",
    )
    parser.add_argument(
        "output_file",
        nargs="?",
        default="/home/kapelinski/Documents/tool/vulnnet_scans_openvas_compilado.csv",
        help="Arquivo CSV de saída compilado",
    )

    args = parser.parse_args()
    compile_csvs(Path(args.input_dir), Path(args.output_file))
